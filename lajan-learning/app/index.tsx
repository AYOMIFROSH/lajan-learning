...
export default function LandingScreen() {
  ...
  const { isAuthenticated, user, autoLoginAttempted, isOnboardingComplete } = useAuthStore();
  ...
  useEffect(() => {
    ...
    const getNextOnboardingStep = () => {
      if (!user?.learningStyle) {
        return '/onboarding'; // Ensure this route exists
      }
      if (!user?.preferredTopics || user.preferredTopics.length === 0) {
        return '/onboarding/topics'; // Ensure this route exists
      }
      if (user?.knowledgeLevel === undefined) {
        return '/onboarding/knowledge-assessment'; // Ensure this route exists
      }
      return '/(tabs)'; // Default to main tabs if onboarding is complete
    };
